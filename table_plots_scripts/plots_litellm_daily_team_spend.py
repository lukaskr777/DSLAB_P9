...
Writes figures to figs/litellm_daily_team_spend.
"""
...
def plot_all_litellm_daily_team_spend(
    ...
    for key, fname, title in [
        ("team_id", f"top{top}_teams_spend.png", f"Top {top} teams by total spend"),
        ...
        if not s.empty:
            bar(
                s, title=title, 
                xlabel=key, ylabel="total spend", fname=fname, outdir=out, 
                top=len(s)
            )
    ...
    # ---- Per-team daily spend lines (Top-N teams) ----
    team_spend = (
        df.groupby("team_id", dropna=False)["spend"]
        .sum()
    ...
    if top_teams:
        per_team_daily = (
            df[df["team_id"].isin(top_teams)]
            .groupby(["date", "team_id"], as_index=False)
            .agg(spend=("spend", "sum"))
            .sort_values(["team_id", "date"], kind="stable")
        )
        for team in top_teams:
            sub = per_team_daily[per_team_daily["team_id"] == team]
            safe = sanitize_fname(f"daily_spend_team_{team}.png")
            line(
                "date", "spend", sub, f"Daily spend for team {team}", 
                "date", "spend", safe, out
            )
